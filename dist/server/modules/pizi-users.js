...
module.exports = ({ config, console, serverLibs }) => {
    ...
    const getCleanUser = (user, validationPropsAllowed = false) => {
        ...
        if (validationPropsAllowed)
            allowedProps.push('checkPassword', 'keyCode', 'urlCode');
        const cleanUser = {};
    ...
    const updateUser = (login, user, callback) => {
        if (!login || !user.checkPassword)
            callback(OperationError);
        else
            UserModel.findOne({ login }, (err, model) => {
                if (err || !model)
                    callback(OperationError);
                else if (!utils.compareEncrypted(user.checkPassword, model.get('password')))
    ...
    const sendCode = (userLogin, userData, generateUrlCode, callback) => {
        ...
        if (serverLibs.email)
            serverLibs.email.sendMail(email, (err, result) => {
                if (err)
                    callback(EmailError);
                else {
    ...
    const checkCode = (login, checkCode, urlCode, callback) => {
        if (urlCode && checkCode) {
            ...
            if (login && CHECK_CODE_SENT[login].attempt < 0)
            ...
            CHECK_CODE_SENT[login].attempt--;
            if (CHECK_CODE_SENT[login].attempt <= 0)
                delete CHECK_CODE_SENT[login];
    ...
    return express.Router().post("/createUser", (req, res) => {
        ...
        if (!req.user || req.params.login !== req.user.user)
        ...
        if (CHECK_CODE_SENT[req.params.userLogin]) {
            checkCode(req.params.userLogin, req.body.checkCode, null, (err, user) => {
                if (err)
                    ...
                    updateUser(req.params.userLogin, Object.assign({ checkPassword: req.body.checkPassword }, user), (err, message) => err ? utils.throwError(err, res) : res.json(message));
            });
        }
        ...
        if (!req.user || req.user.user !== req.params.userLogin)
            utils.throwError(NotAuthorizedError, res);
        else
            UserModel.findOne({ login: req.params.userLogin }, (err, model) => {
                try {
                    if (err)
                        throw OperationError;
                    else if (!utils.compareEncrypted(req.body.checkPassword, model.get('password')))
                        throw NotAuthorizedError;
                    else
                        UserModel.deleteOne({ login: req.params.userLogin }, (err, model) => {
                            if (err)
                                utils.throwError(OperationError, res);
                            else
        ...
        if (req.body.urlCode && req.body.checkCode) {
            checkCode(null, req.body.checkCode, req.body.urlCode, (err, user) => {
                if (err)
                    ...
                    UserModel.findOne({ email: user.email }, (err, model) => {
                        try {
                            if (err || !model)
                                throw OperationError;
            ...
            UserModel.findOne({ email: req.body.email }, (err, model) => {
                if (err)
                    throw utils.throwError(OperationError, res);
                if (model)
                    sendCode(model.login, { email: req.body.email }, true, (err, message) => err ? utils.throwError(err, res) : res.json(message));
                else
                    res.json({ message: 'ok' });
            });
        }
...
//# sourceMappingURL=pizi-users.js.map