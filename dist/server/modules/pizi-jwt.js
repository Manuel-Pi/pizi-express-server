    ...
    router.use((req, res, next) => {
        ...
        if (auth && auth.match(/^Bearer ((\w|-|_)+.(\w|-|_)+.(\w|-|_)+)$/)) {
            ...
            jwt.verify(token, config.token.key, (err, decoded = {}) => {
                if (err)
                    utils.throwError(err.message, res);
                else {
                    // Check user exist
        ...
        const userRoles = utils.getUserRoles((_a = req.user) === null || _a === void 0 ? void 0 : _a.role);
        ...
        if (req.headers.login && req.headers.password) {
            check(req.headers.login, req.headers.password, (err, user) => {
                if (err === "wrong password")
                    ...
                    utils.throwError(BadCredential, res);
                else {
            ...
            utils.throwError(NoCredential, res);
    });
    return router;
};
/**
...
const checkAuthDefault = (login, password, callback) => {
    try {
        UserModel.findOne({ "login": login }, (err, model) => {
            ...
            else if (model && utils.compareEncrypted(password, model.get('password')))
...
//# sourceMappingURL=pizi-jwt.js.map