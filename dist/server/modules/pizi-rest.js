...
module.exports = ({ config = { restrictions: {} }, console }) => {
    ...
    const routerREST = express.Router().use((req, res, next) => {
        ...
        if (req.path === "/")
            ...
            if (restriction && restriction.attributes)
                restriction.attributes = Object.assign(Object.assign({}, defaultRestrictions.attributes), restriction.attributes);
            restriction = Object.assign(Object.assign({}, defaultRestrictions), config.restrictions[req.store]);
            const roleNeeded = restriction.all || restriction[req.method.toLowerCase()];
            if (roleNeeded && !utils.getUserRoles((_a = req.user) === null || _a === void 0 ? void 0 : _a.role).has(roleNeeded))
                allowed = false;
            // Default Attributes
    ...
    routerREST.route(/^\/\w*$/).post((req, res) => {
        try {
            ...
            model.save((err) => {
                if (err)
                    utils.throwError(OperationError, res, { logger: console });
                else
...
//# sourceMappingURL=pizi-rest.js.map