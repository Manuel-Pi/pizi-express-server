...
const CACHE_INDEX = JSON.parse(process.env.CACHE_SSR_APP_FILES);
function renderApp(serverAppPath, appElement) {
    return async function renderApp(req, res, next) {
        try {
            ...
            if (!CACHE_INDEX) {
                ...
                const serverAppComponentPath = path.join(serverAppPath, `/ssr/server.js?${Date.now()}`);
...
function StaticRendering(req, res, serverAppPath) {
    const appIndexFilePath = path.join(serverAppPath, "/client/index.html");
    res.sendFile(appIndexFilePath);
}
async function SSRRendering(req, res, serverAppPath, appElement) {
...
export default async function (appDir) {
    ...
    if (App)
        logger.info(`ssr active for '${appDir}'`);
...
//# sourceMappingURL=singlePageAppRouter.js.map