module.exports = ({ socketServer, console }) => {
    ...
    nsp.on('connection', (socket) => {
        ...
        socket.on('addRoom', (room) => {
            if (!room)
                return;
            room = Object.assign({ allowedUsers: [], users: [], private: false, owner: null }, room);
            const [user, namespace] = Namespace.getDataFromSocket(socket);
            if (!namespace)
                return;
            ...
            if (existingRoom)
                namespace.joinRoom(existingRoom.id, user, room => {
                    socket.join(room.id);
                    socket.emit('roomData', room.getJson());
                    socket.broadcast.to(room.id).emit('userJoinRoom', { name: room.name, user: user.name });
                    updateRoomForUsers(namespace);
            ...
            // Create room
            if (!existingRoom)
                namespace.createRoom(room, createdRoom => {
...
//# sourceMappingURL=pizi-chat.js.map