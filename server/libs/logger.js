...
class Logger {
    ...
    static LOG_FILES = {}
    /* Console log level */
    static CONSOLE_LEVEL = 1
    /* Time format */
    static TIME_FORMAT = new Intl.DateTimeFormat('en-US', {
        year: 'numeric', month: 'numeric', day: 'numeric',
        hour: 'numeric', minute: 'numeric', second: 'numeric',
        hour12: false
    })
    /* Log info string */
    static getInfo = (name) => "[" + Logger.TIME_FORMAT.format(new Date()) + "]" + name ? "[" + name + "]" : ""
    /* Get log level, numeric value */
    static getLogLevelValue = (logLevel) => {
        switch(logLevel){
            case "debug":
                return 0
            case "info":
    ...
    static getConfig = (name) => Logger.CONFIG.loggers[name] || null
    ...
    static logToFile(filename, message){
        ...
        if(!Logger.LOG_FILES[filename]){
            ...
            if(!Logger.LOG_FILES[filepath]) Logger.LOG_FILES[filepath] = fs.createWriteStream(filepath, {flags : 'a'})
        }
    ...
    debug(...args){
        ...
        if(Logger.getLogLevelValue(config.level) > 0) return
        const info = "DEBUG" + Logger.getInfo(this.name)
        if(Logger.CONSOLE_LEVEL <= 0) console.debug('\x1b[37m' + info, ...args, '\x1b[0m')
        Logger.logToFile(config.file, [info, ...args].join(" "))
    }

    info(...args){
        const config = this.config()
        if(Logger.getLogLevelValue(config.level) > 1) return
        const info = "INFO" + Logger.getInfo(this.name)
        if(Logger.CONSOLE_LEVEL <= 1) console.info('\x1b[36m' + info, ...args, '\x1b[0m')
        Logger.logToFile(config.file, [info, ...args].join(" "))
    }

    warn(...args){
        const config = this.config()
        if(Logger.getLogLevelValue(config.level) > 2) return
        const info = "WARN" + Logger.getInfo(this.name)
        if(Logger.CONSOLE_LEVEL <= 2) console.warn('\x1b[33m' + info, ...args, '\x1b[0m')
        Logger.logToFile(config.file, [info, ...args].join(" "))
    }

    error(...args){
        const config = this.config()
        if(Logger.getLogLevelValue(config.level) > 3) return
        const info = "ERROR" + Logger.getInfo(this.name)
        if(Logger.CONSOLE_LEVEL <= 3) console.error('\x1b[31m' + info, ...args, '\x1b[0m')
        Logger.logToFile(config.file, [info, ...args].join(" "))
    }
...
module.exports = {
    ...
    getLogger(name){
        if(!LOGGERS.get(name)) LOGGERS.set(name, new Logger(name))
