...
function checkRights(rights: string[] = []){
    return rbac(req => ({[req.params.store]: rights}))
}

function getModel(store: string){
    for(const ModelClass of MODELS) if(ModelClass.name.toLowerCase() + "s" === store) return ModelClass
    throw new HttpErrors.NotFound()
}

function getAdapter(store: string){
    return db(getModel(store))
}

router.route('/:store').get(checkRights(['read']), async (req, res, next) => {
    try{
        const userRequest = req as any as UserRequest
        ...
        const models: Model[] = await dbAdapter.find(allowedIds.length ? {id: allowedIds} : {})
