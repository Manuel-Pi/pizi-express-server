...
export function withRefreshToken(req: OAuth2Server.Request): boolean{
...
router.post("/token", async (req, res, next) => {
    try{
        ...
        res.status(200).json(tokenToSnakeCase(token, withRefreshToken(req as OAuth2Server.Request)))
    } catch(e){
        next(e)
    }
})
...
router.get("/authorize", async (req, res, next) => {
    try{
        res.redirect(302, `${process.env.OAUTH_LOGIN_URL}?${new URLSearchParams(req.query as any)}`)
    } catch(e){
        next(e)
    }
})
...
router.post("/authorize", async (req, res, next) => {
    try{
        ...
        if(req.body.stayConnected) STAY_CONNECTED_CACHE_ARRAY.add(authorizationCode.authorizationCode)
