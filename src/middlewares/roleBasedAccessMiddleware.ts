...
export default function(requiredRightsFunction: (req: Request) => { [rightType: string]: string[]} ) {
    return async function(req: Request, res: Response, next: NextFunction){
        try{
            ...
            if(!userReq.token?.user?.id) throw new HttpErrors.Unauthorized(`user not found in request`)
            ...
            for(const requiredRightType of Object.keys(requiredRights)){
                ...
                for(const requiredRightName of requiredRights[requiredRightType]){
                    if(!userRights[requiredRightType][requiredRightName]) throw new HttpErrors.Unauthorized(`user do not have '${requiredRightType}.${requiredRightName}' right`)

                    // Check restrictions
                    const rightsDefinition: any = RIGHTS_DEFINITION
                    for(const restrictionKey of rightsDefinition[requiredRightType][requiredRightName].restrictions){
                        switch(restrictionKey){
                            case "id":
                                ...
                                if(!id) throw new Error(`cannot find an id for restrictions`)
                                // Authorized if user restriction ids is an empty array
                                if(!userAllowedIds.length) continue
                                // Authorized if user restriction ids match required
                                if(userAllowedIds.includes(id)) continue
                            default:
