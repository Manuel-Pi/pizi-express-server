...
interface IdRestriction {
    ids: string[]
}

export interface IdRight extends IRight {
    restrictions: IdRestriction
}

export function isValidRight(right: any = {}){
    const rightsDefinition: any = RIGHTS_DEFINITION
    const rightTypes = Object.keys(rightsDefinition)

    // Check if right exist
    if(!rightTypes.includes(right.type)) throw new Error(`invalid right type`)
    if(!rightsDefinition[right.type][right.name]) throw new Error(`invalid right name`)
